...
use app\modules\quanly\models\DuongThuySearch;
...
/**
 * DuongThuyController implements the CRUD actions for DuongThuy model.
 */
class DuongThuyController extends \app\modules\quanly\base\QuanlyBaseController
{

    public $title = "Đường thủy";

    /**
     * Lists all DuongThuy models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new DuongThuySearch();
    ...
    /**
     * Displays a single DuongThuy model.
     * @param integer $id
     * @return mixed
     */
        ...
        if ($model->load($request->post()) && $model->save() && $filedinhkem->load($request->post())) {
            ...
            if($filedinhkem->fileupload != null){
                ...
                foreach($filedinhkem->fileupload as $i => $item){
                    ...
                    $file[] = 'uploads/duong-thuy/'.$model->id.'/'.$item->baseName.'.'.$item->extension;
                    $path = 'uploads/duong-thuy/'.$model->id.'/';
            ...
            Yii::$app->db->createCommand("UPDATE duong_thuy SET geom = ST_SETSRID(ST_GeomFromText(ST_AsText(ST_GeomFromGeoJSON('" . $model->geojson . "'))),4326) WHERE id = :id")
    ...
    /**
     * Updates an existing DuongThuy model.
     * For ajax request will return json object
    ...
    /**
     * Delete an existing DuongThuy model.
     * For ajax request will return json object
    ...
    /**
     * Finds the DuongThuy model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return DuongThuy the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DuongThuy::findOne($id)) !== null) {
            return $model;
        } else {
