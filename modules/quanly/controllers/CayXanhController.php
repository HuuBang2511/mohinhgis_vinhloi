...
use app\modules\quanly\models\CayXanhSearch;
...
/**
 * CayXanhController implements the CRUD actions for CayXanh model.
 */
class CayXanhController extends \app\modules\quanly\base\QuanlyBaseController
{
    ...
    /**
     * Displays a single CayXanh model.
     * @param integer $id
     * @return mixed
     */
        ...
        if($model->load($request->post()) && $model->save() && $filedinhkem->load($request->post())){
            ...
            if($filedinhkem->fileupload != null){
                ...
                foreach($filedinhkem->fileupload as $i => $item){
                    ...
                    $file[] = 'uploads/cay-xanh/'.$model->id.'/'.$item->baseName.'.'.$item->extension;
                    $path = 'uploads/cay-xanh/'.$model->id.'/';
    ...
    /**
     * Updates an existing CayXanh model.
     * For ajax request will return json object
    ...
    /**
     * Delete an existing CayXanh model.
     * For ajax request will return json object
    ...
    /**
     * Finds the CayXanh model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CayXanh the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = CayXanh::findOne($id)) !== null) {
            return $model;
        } else {
